...
public:
    Simplex(const vector<vector<Fraqtion>> &a, const vector<Fraqtion> &b, const vector<Fraqtion> &c, SimplexMode mode, char varName, char funcName, int padding = 0);
...
Simplex::Simplex(const vector<vector<Fraqtion>> &a, const vector<Fraqtion> &b, const vector<Fraqtion> &c, SimplexMode mode, char varName, char funcName, int padding) {
    this->mode = mode;
...
void Simplex::PrintTask() const {
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < n; j++) {
            Fraqtion aij = table[i][n + m] >= 0 ? table[i][j] : -table[i][j];
...
void Simplex::PrintTable() const {
    ...
    for (int i = 0; i < m; i++) {
        cout << string(padding, ' ') << "|   " << varName << left << setw(4) << (basis[i] + 1) << " |" << right;
        PrintVector(table[i]);
    }

    PrintLine();
    cout << string(padding, ' ') << "|    " << funcName << "    |";
    PrintVector(deltas);
    PrintLine();
}
...
vector<SimplexSolve> Simplex::SolveIntegerBranchesAndBorders(bool debug, int depth) {
    ...
    Simplex simplex1(a1, b1, initialC, mode, varName, funcName, padding + 6);
    Simplex simplex2(a2, b2, initialC, mode, varName, funcName, padding + 6);

    cout << string(padding, ' ') << "Divide to tasks: " << varName << (realIndex + 1) << " <= " << b << " and " << varName << (realIndex + 1) << " >= " << (b + 1) << endl;
